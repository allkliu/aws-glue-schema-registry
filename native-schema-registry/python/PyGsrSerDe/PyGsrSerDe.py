...
class GlueSchemaRegistrySerializer:
    def __init__(self):
        self.serializer = GsrSerDe.glue_schema_registry_serializer()

    def __del__(self):
        del self.serializer

    def encode(self, transport_name: str, schema: GlueSchemaRegistrySchema, byte_arr: bytes) -> bytes:
        ro_byte_array = GsrSerDe.read_only_byte_array(byte_arr)
        gsr_schema = GsrSerDe.glue_schema_registry_schema(schema.schema_name, schema.schema_def, schema.data_format)
...
class GlueSchemaRegistryDeserializer:
    def __init__(self):
        self.deserializer = GsrSerDe.glue_schema_registry_deserializer()
